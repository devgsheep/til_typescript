...
- 비동기 처리는 시간이 많이 걸리는 작업 진행중에 다른 일도 `병렬로 처리`하도록 함
...
### 4.4. 쿼리를 전송할 시에는 5가지 방식으로 보낼 수 있다.

- `REST API` 칭함 (주소/라우터)
- `CRUD` 작업(DB를 Create, Reade, Update, Delete)

- `GET` : 자료를 주세요. (DB에서 자료 읽고 결과 회신)
- `POST` : 자료를 전송합니다. (DB에서 자료 1개 추가)
- `DELETE` : 자료를 삭제하세요. (DB에서 자료 1개 삭제)
- `PUT` : 하나의 자료내용 전부를 교체하세요. (DB에서 자료 한개 전체 수정)
- `PATCH` : 하나의 자료내용 중 한 부분만 수정하세요. (DB에서 자료 한개 중 일부만 수정)
      ...
      function getPhotos() {
        ...
        xhr.onload = function () {
          if (xhr.status === 200) {
            ...
            console.log("서버가 전원이 꺼졌습니다. 다시 시도해 주세요.");
          }
        };
      }
      // getPhotos();
...
- 서버 연동이 끝날 때, 성공 함수와 실패함수 2개를 매개변수로 받아서 실행
- 2개의 함수는 서버연동이 완료되면 자동실행 되도록 구성.

### 5.1. Promise는 2개의 매개변수 (즉 콜백함수)를 받음

- resolve 콜백함수 : 백엔드 정상 결과 처리 함수
- reject 콜백함수 : 백엔드 오류 결과 처리 함수

### 5.2. Promise는 3가지의 상태가 있습니다.
...
- 2단계 : 반드시 function 앞에 async를 붙여줍니다.

```js
async function getAllData() {}
```

- 3단계 : 반드시 function 안쪽에 try ~ catch를 작성합니다.
...
- 4단계 : 실행하려는 함수 반드시 try 블럭 안쪽에 배치하고, 앞에 await을 붙인다.

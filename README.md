...
- Primitive 데이터 : number, string, null, undefined, symbol, boolean
- any, unknown, never

- 기본형 데이터가 너무 많다면 묶어서 관리하자.
  - 이를 참조 타입, 즉 Reference 타입이라고 한다.
  - Reference타입을 객체 라고 한다.
...
- 원시데이터를 묶어서 관리한다.
- 데이터 각 요소의 `이름, 즉 속성(Property)를 정해서` 관리한다.
- 아래 문장을 `객체 리터럴`이라고 하며, 많이 중요함

...
- switch 문의 값은 직접 원시값으로 작성 시 오류발생이 가능
- TypeScript에 있는 `enum`을 사용하시길 권장
- enum은 정해진 값만 사용하게 권장할 때(열거형)
- 인터넷 상태 등에 대한 내용을 코드할 때 많이 사용
...
while (count < 5) {
  // 실행하라.
  count += 1; // 반드시 조건을 거짓으로 만들도록 코드 필요.
}
```

## 3. do while

- `일단 한번은 실행`, 그리고 `조건이 참` 이면서 `반복 횟수를 모를 때`사용

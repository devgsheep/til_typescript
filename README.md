...
- 타이핑으로 객체 `{}` 를 적어서 만든다 라고 해서 `객체 리터럴`이라고 함.
...
- 많이 복잡합니다. 추후 필요로 한 것은 class라는 문법입니다.
- type을 추가하면 가독성이 좋다.
...
- 절대로 주의하셔야 합니다. (객체 생성하고 싶은 경우 `new`를 꼭 붙인다.)
...
function PersonInfo(this: PersonInfoType): void {
  ...
  this.age = 20;
  this.job = "소설가";
  // Default로 this가 리턴됩니다.
  // return this;
}

// ts 에서는 new는 class로 생성을 원활히 지원한다.
...
- 아래 처럼 함수를 실행하면 `this`가 window가 된다.
...
type Bpmb = { 이름: string; 생일: string };
const 블랙핑크_멤버_제니: Bpmb = { 이름: "제니", 생일: "96-01" };
const 블랙핑크_멤버_로제: Bpmb = { 이름: "로제", 생일: "97-02" };
const 블랙핑크_멤버_지수: Bpmb = { 이름: "지수", 생일: "95-01" };
블랙핑크_멤버_지수.이름; // 지수
블랙핑크_멤버_지수["이름"]; // 지수

const 블랙핑크_멤버_리사: Bpmb = { 이름: "리사", 생일: "97-03" };
...
const 블랙핑크_멤버_제니 = {
  ...
  //기능 Method
  sing: function () {
    console.log(`${this.이름}가 노래합니다.`); // this는 scope 안쪽을 가리킴
  },
  dance: function () {
...
### 6.3. 객체에 속해 있는 메소드를 `축약해서 생성`하는 문법
...
```js
const 아이유 = { job: "가수" };
아이유 = "10살"; // const는 불변합니다.

// 아래는 참조로 접근하므로 변경이 가능하다.
// 아이유가 const이지, job은 const가 아니다.
아이유.job = "연기자"; // 됩니다.
...
- `객체 구조 분해 할당` 강력 추천(리엑트부터 무조건 사용)

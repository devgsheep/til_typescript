...
```ts
// OR 연산자(또는) : 하나만 true이면 된다.
const result: boolean = true || true;
...
- 기본 값을 셋팅 할 때 활용
- null, undefined가 아닌 것을 찾아서 셋팅

```ts
let userName: string; // 현재 undefined
let displayName: string = userName ?? "Guest";
// userName이 없으면 Guest, 현재 값 Guest

let userName: string = "hong"; // 현재 undefined, null이 아님
let displayName: string = userName ?? "Guest";
// hong
```
...
console.log(typeof arr); // number[] 실제로는 object라고 출력 됨

const obj: { age: number } = { age: 10 };
console.log(typeof obj); // {age:number} 실제로는 object라고 출력 됨

type T = { age: number };
const obj: T = { age: 10 };
console.log(typeof obj); // T 실제로는 object라고 출력 됨
```
- `기본형을 제외하고 참조형 데이터들은 직접 비교를 하는 방안을 찾아야 한다.`

...
class Child extends Parent {
  ...
  constructor(age: number, name: string) {
    super(name); // === new Parent("엄마"), 초기화 담당
    this.age = age;
  }
  // 메서드
  sing() {
...
const iuMom = new Parent("이아무개");

// Child는 Parent를 확장(extends)
const iu = new Child(28, "김아무개");

// 상속받은 자식은 값으로서 부모에게 할당할 수 있다.
const mom: Parent = c;

// 상속받은 자식에게 부모는 값으로 할당할 수 없다.
const son: Child = p; // Error
```
...
// 상황이 달라진다.
const 모든동물: Animal = 야옹이;
// Cat은 필수 속성인 age가 필요하지만 Animal에는 없다.
const 옹이: Cat = 동물; // Error (age 속성이 없음)
```
...
// Dog은 필수 속성인 breez가 필요하지만 Animal에는 없다.
const 멍이: Dog = 동물; // Error (breez 속성이 없음)
```

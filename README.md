...
- 클래스 `설계도` 이다.
- 클래스 즉, 설계도를 기반으로 `객체(결과물-instance)`을 만드는 것
- 목적은 `인스턴스`, 즉 `new를 통해서 객체`를 대량 생산하는 것
- class는 문법으로 존재하며, 실무에서 직접 `class를 만드는 경우`는 극히 드물다.
- 백엔드를 js로 만들 때는 이해 필수입니다.
...
## 2. class에서 new하면 실행되는 함수는 약속

- constructor 메서드 : 디폴트 생성자 메서드

```js
class 클래스명 {
  // 작성 안하셔도 기본적으로 new하면 작동됩니다.
  constructor() {}
}
const 인스턴스 = new 클래스명();
...
- constructor 메서드 : 속성의 초기값을 세팅해 줄 수 있어요.
...
class Cat extends Animal {
  ...
  show() {
    this.eye; // public이라서 마음대로 접근
    this.nose; // Error, private 이라서 아무도 접근 못함
    this.lips; // protected는 자식이라서 허용됨
  }
}
const cat = new Cat();
cat.eye; // public 이라서 마음대로 접근
cat.nose; // Error, private 이라서 아무도 접근하지 못함
cat.lips; // Error, protected 라서  아마도 접근하지 못함
```

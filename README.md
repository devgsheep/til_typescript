...
type IsMemer = boolean;
const isMember: IsMemer = true;
```

- 추후 진행시에는 type에 대한 정의를 먼저 고민해보자.
...
const iu: Student = { name: "아이유", age: 28, major: "가수" };
```

### 2.3. `확장`이 가능하다. (기존 type을 확장해서 또 다른 type 작성)
...
const cat: MyPet = { eye: 2, mustache: true };
```
